...
// Bike model faces -Z by default; rotate group to point nose toward travel dir
const DIR_ROT = { RIGHT: Math.PI/2, LEFT: -Math.PI/2, DOWN: Math.PI, UP: 0 };
...
scene.fog = new THREE.Fog(0x010108, 12, 72);

const camera = new THREE.PerspectiveCamera(80, 1, 0.05, 200);

// Bloom post-processing — this is what makes the TRON glow work
...
function createBike(hexColor) {
  ...
  // ─ Body panels ─
  box(0.56,0.13,1.80, dark(),  0,0.13, 0);          // lower chassis
  box(0.36,0.17,1.32, dark(),  0,0.32, 0.04);       // upper fairing
  box(0.28,0.13,0.58, glass(), 0,0.46,-0.1);        // cockpit canopy

  // ─ Glow trim lines ─
  box(0.58,0.022,1.82, glow(), 0,0.022, 0);         // bottom edge
  box(0.38,0.022,1.34, glow(), 0,0.412, 0.04);      // top edge
  box(0.022,0.15,1.82, glow(),-0.29,0.13, 0);       // left side strip
  box(0.022,0.15,1.82, glow(), 0.29,0.13, 0);       // right side strip
  box(0.58,0.15,0.022, glow(), 0,0.13,-0.91);       // front glow bar
  box(0.58,0.15,0.022, glow(), 0,0.13, 0.91);       // rear (trail origin)

  // ─ Wheel (large TRON-style disc) ─
  const wm = new THREE.Mesh(
    new THREE.CylinderGeometry(0.23,0.23,0.07,28), dark()
  );
  wm.rotation.z = Math.PI/2; wm.position.set(0,0.23,0.1);
  group.add(wm);

  // Wheel rim glow
  const rim = new THREE.Mesh(new THREE.TorusGeometry(0.23,0.018,8,28), glow());
  rim.rotation.y = Math.PI/2; rim.position.set(0,0.23,0.1);
  group.add(rim);

  // Spokes (4, in the wheel plane = YZ plane since wheel axis is X)
  for (let i=0; i<4; i++) {
    box(0.014,0.46,0.014, glow(), 0,0.23,0.1, (i/4)*Math.PI,0,0);
  }
...
// ── Trail wall geometry ────────────────────────────────────────────────────
// Flat, thin walls oriented perpendicular to direction of travel (TRON Legacy style)
...
function buildArena(g) {
  ...
  // TRON Legacy-style grid: bright lines that bloom into glowing blue
  const lineMat = new THREE.LineBasicMaterial({ color: 0x0033aa });
  const pts = [];
  for (let i=0; i<=g; i++) {
    pts.push(i,0.003,0, i,0.003,g);
    pts.push(0,0.003,i, g,0.003,i);
  }
  const lineGeo = new THREE.BufferGeometry();
  lineGeo.setAttribute('position', new THREE.Float32BufferAttribute(pts,3));
  scene.add(new THREE.LineSegments(lineGeo, lineMat));

  // Faint secondary grid (half spacing) for depth
  const lineMat2 = new THREE.LineBasicMaterial({ color: 0x001155 });
  const pts2 = [];
  for (let i=0; i<=g*2; i++) {
    pts2.push(i*.5,0.002,0, i*.5,0.002,g);
    pts2.push(0,0.002,i*.5, g,0.002,i*.5);
  }
  const lineGeo2 = new THREE.BufferGeometry();
  lineGeo2.setAttribute('position', new THREE.Float32BufferAttribute(pts2,3));
  scene.add(new THREE.LineSegments(lineGeo2, lineMat2));

  // Border walls — tall, bright, glow heavily
...
// ── Game state ─────────────────────────────────────────────────────────────
let myId=null, players={}, lastTick=0, tickMs=100, gameActive=false, trailOn=true;

// ── Camera (first-person cockpit view) ─────────────────────────────────────
...
function updateCamera() {
  ...
  const d = DIR_VEC[me.dir];
  _look.set(wx+d.x*10, CAM_H*0.55, wz+d.z*10);
  camera.lookAt(_look);
}
...
socket.on('game-start', data => {
  myId=data.myId; tickMs=data.tickMs; trailOn=true;
  startGame(data);
  music.start();
});
...
socket.on('tick', data => {
  ...
  for (const pd of data.players) {
    ...
    if (!p) continue;
    // Add trail wall at previous position if trail was active
    ...
    p.prevX=p.targetX; p.prevZ=p.targetZ;
    p.targetX=pd.x; p.targetZ=pd.y;
    p.dir=pd.dir; p.alive=pd.alive;
    if (!pd.alive) {
      p.mesh.visible=false;
      document.getElementById('chip-'+pd.id)?.classList.add('dead');
    } else if (pd.id!==myId) {
      p.mesh.position.set(pd.x,0,pd.y);
      p.mesh.rotation.y=DIR_ROT[pd.dir];
    }
  }
});
...
socket.on('game-over', data => {
  ...
  if (data.winner) {
    ...
    msg.textContent='DRAW'; msg.style.color='#aaa'; msg.style.textShadow='none';
  }
...
function startGame(data) {
  ...
  for (const pd of data.players) {
    ...
    mesh.rotation.y = DIR_ROT[pd.dir];
    if (pd.id===myId) mesh.visible=false; // first-person: hide own bike
    scene.add(mesh);
    players[pd.id]={color:pd.color,dir:pd.dir,
      targetX:pd.x,targetZ:pd.y,prevX:pd.x,prevZ:pd.y,
...
function updateTrailHUD() {
  const el = document.getElementById('trail-toggle-btn');
  if (!el) return;
  el.textContent = trailOn ? 'TRAIL: ON' : 'TRAIL: OFF';
  el.style.borderColor = trailOn ? '#00e5ff' : '#ff1744';
  el.style.color        = trailOn ? '#00e5ff' : '#ff1744';
  el.style.boxShadow    = trailOn
    ? '0 0 8px #00e5ff' : '0 0 8px #ff1744';
}
...
document.addEventListener('keydown', e => {
  ...
  if ((e.key===' '||e.key==='Shift') && gameActive) { e.preventDefault(); socket.emit('toggle-trail'); return; }
  const dir=KEY_MAP[e.key];
  if (!dir||!gameActive) return;
  e.preventDefault();
  const me=players[myId];
  if (!me||!me.alive||dir===OPP[me.dir]) return;
  me.dir=dir; // local prediction for immediate camera response
  socket.emit('turn',{dir});
});

// ── Synthesized music (inspired by End of Line — BPM 128, B minor) ─────────
const music = (() => {
  let ac=null, master=null, running=false, muted=false, nextBar=0;
  const BPM=128, B=60/BPM;

  function noise(s) {
    const n=Math.ceil(ac.sampleRate*s), buf=ac.createBuffer(1,n,ac.sampleRate);
  ...
  function kick(t) {
    ...
    o.frequency.setValueAtTime(200,t); o.frequency.exponentialRampToValueAtTime(0.001,t+0.35);
    g.gain.setValueAtTime(3,t); g.gain.exponentialRampToValueAtTime(0.001,t+0.35);
  ...
  function bass(t,freq,dur) {
    ...
    o.type='sawtooth'; o.frequency.value=freq; f.type='lowpass'; f.Q.value=6;
  ...
  function scheduleBar(t) {
    ...
    for (let i=0;i<16;i++) hihat(t+i*B/4, i%4===0?.4:i%2===0?.25:.12);
    // Bass: B minor pentatonic  B1=61.7  D2=73.4  F#2=92.5  E2=82.4
    [[0,61.74],[1,73.42],[2,92.50],[2.5,82.41],[3,73.42],[3.5,61.74]].forEach(([dt,f])=>bass(t+dt*B,f,B*.42));
    // Lead arpeggio: B3 D4 F#4 A4 pattern
    const A=[246.94,293.66,369.99,440,246.94,369.99,440,523.25,
             246.94,293.66,369.99,440,523.25,440,369.99,293.66];
    for (let i=0;i<16;i++) arp(t+i*B/4, A[i], B/4*.65);
  }
  function pump() {
    if (!running) return;
  ...
  return {
    start() {
      ...
      running=true; nextBar=ac.currentTime+0.05; pump();
    },
...
document.getElementById('trail-toggle-btn').addEventListener('click',()=>socket.emit('toggle-trail'));
